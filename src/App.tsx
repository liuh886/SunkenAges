...
export default function App(): JSX.Element {
  ...
  const publish = () => {
    const result = scoreExhibition(claim, selected);
    setOutcome(result);
  ...
  return (
    <main className="app-shell">
      ...
      {phase === "briefing" ? <Briefing notice={notice} onStart={startDive} /> : null}
      {phase === "dive" ? (
        <DiveScreen
          hud={hud}
          onHud={setHud}
      ...
      {phase === "result" && outcome ? <Result outcome={outcome} onRestart={restart} /> : null}
...
function Briefing({ notice, onStart }: { notice: string | null; onStart: () => void }): JSX.Element {
  return (
    <section className="screen briefing-screen">
      <div className="briefing-copy">
        <p className="section-index">01 / 首次调查</p>
        <h1>今天打捞的，<br />可能不是宝藏。</h1>
        <p className="lead">
          浅海航道发现一艘沉没于海侵初期的民用运输船。官方记录称那是一场“有序撤离”，但船体位置与航线档案并不吻合。
        </p>
        {notice ? <p className="notice">{notice}</p> : null}
        <div className="mission-facts" aria-label="任务信息">
          <div><span>目标海域</span><strong>盐雾航道 · 42 m</strong></div>
          <div><span>任务</span><strong>回收三件可验证证据</strong></div>
          <div><span>限制</span><strong>氧气 100 · 载重 6</strong></div>
        </div>
      ...
      <div className="briefing-visual" aria-label="盐雾航道声呐记录">
        ...
        <div className="museum-window">
          <div className="window-header"><span>盐雾航道</span><span>信号强度 73%</span></div>
          ...
          <div className="window-footer"><span>民用船只 / 约 312 年前</span><span>档案冲突</span></div>
        </div>
        <blockquote>“游客喜欢完整的故事。可惜历史通常不配合。”<cite>— 馆长助理弥洛</cite></blockquote>
      </div>
    </section>
  );
}
  ...
  return (
    <section className="screen dive-screen" aria-label="深海打捞场景">
      <div className="dive-hud">
        ...
        <div className="hud-cluster location-cluster"><small>盐雾航道</small><strong>深度 {String(hud.depth).padStart(2, "0")} m</strong></div>
        <div className="hud-cluster cargo-cluster"><span>载重</span><strong>{hud.cargo} / 6</strong></div>
      </div>
      ...
      <div className="dive-bottom-bar">
        <div className="toolbelt" aria-label="打捞工具">
          <span><b>01</b> 扫描仪</span><span><b>02</b> 密封箱</span><span><b>03</b> 微型切割器</span>
        </div>
      ...
      <div className="mobile-controls" aria-label="移动端控制">
        ...
        <button type="button" className="action-control" onPointerDown={() => controllerRef.current?.interact()}>打捞</button>
      </div>
    </section>
  );
}
...
function Museum({ claim, recovered, selected, notice, onClaim, onToggle, onPublish }: MuseumProps): JSX.Element {
  const evidence = useMemo(() => EVIDENCE.filter((item) => recovered.includes(item.id)), [recovered]);
  ...
  return (
    <section className="screen museum-screen">
      <aside className="museum-sidebar">
        ...
        <button className="primary-button publish-button" type="button" disabled={!canCurate(selected)} onClick={onPublish}>
      ...
      <div className="investigation-area">
        <div className="timeline-panel">
          <div><small>文明时间线</small><strong>海侵初期 · 312 年前</strong></div>
          <div className="timeline-track" aria-label="远古至文明末期"><span>远古</span><i /><b>海侵初期</b><i /><span>文明末期</span></div>
        </div>
        <div className="evidence-board">
          <div className="board-header">
            <div><small>调查墙 01</small><h3>盐雾航道撤离事件</h3></div>
          ...
          <div className="evidence-grid">
            {evidence.map((item) => (
              ...
              >
                <small>{item.code}</small><strong>{item.name}</strong><p>{item.description}</p><span>{item.tool}</span>
              </button>
            ))}
          </div>
          <div className="board-note"><b aria-hidden="true">!</b><p>证据支持度并不等于真相。星际游客会用自己的文明经验挑战你的解释。</p></div>
        </div>
      </div>
    </section>
  );
...
function Claim({
  ...
  return (
    <button type="button" className={`claim-option ${selected ? "is-selected" : ""}`} role="radio" aria-checked={selected} onClick={onClick}>
      <span>{label}</span><b>{title}</b><small>{description}</small>
    </button>
  );
}

function Result({ outcome, onRestart }: { outcome: ExhibitionOutcome; onRestart: () => void }): JSX.Element {
  return (
    <section className="screen result-screen">
      <div className="result-copy">
        ...
        <button className="primary-button" type="button" onClick={onRestart}>下一调查日 <span aria-hidden="true">→</span></button>
      </div>
      <div className="visitor-panel">
        <div className="visitor-portrait" aria-hidden="true"><span /></div>
        <div><small>星际旅行者 · 奥罗</small><blockquote>{outcome.visitorQuote}</blockquote><p>新调查地点已记录：<b>潮门避难站</b><br />官方地图称它从未投入使用。</p></div>
      </div>
    </section>
  );
}
